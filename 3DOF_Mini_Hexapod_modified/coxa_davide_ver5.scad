...
module coxa(servo_height = 27, servo_clearance = 7) {
	...
	root_radius = 4;
	block(servo_width);
	translate([0, -36, 0]) block(servo_width);
	...
	translate([fork_width/2 - servo_cog_offset, 0, 0]) union() {
		difference() {
			...
			color("white") translate([servo_cog_offset - fork_width/2, -35.5 + fork_height/2, 0]) rotate([90, 270, 180]) servo_arm();
		}
		...
		translate([fork_width/2 + 15/2, 0, -3]) {
			difference() {	
				...
				for(i = [-1, 1]) {
					translate([-1, i * 30/2, 0]) cylinder(r=1.4, h=20, center = true);
				}
...
module block(servo_width)
{
	translate([15, 17, -6])
	{
		cube([5, 2, servo_width]);
	}
}
